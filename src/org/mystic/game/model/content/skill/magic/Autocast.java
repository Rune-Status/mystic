...
public class Autocast {
	...
	public static boolean clickButton(Player player, int id) {
		...
		case 1094:
			if (player.getMagic().getSpellCasting().isAutocasting()) {
				...
				if (player.getMagic().getSpellBookType() == SpellBookTypes.ANCIENT) {
					if (weapon.getId() == 4675 || weapon.getId() == 4710 || weapon.getId() == 6914
							|| weapon.getId() == 15486) {
						sendSelectionInterface(player, weapon.getId());
					} else {
						resetAutoCastInterface(player);
						player.send(new SendMessage("You can only auto-cast ancients with an ancient staff."));
						return false;
					}
				} else {
					sendSelectionInterface(player, weapon.getId());
	...
	public static void sendSelectionInterface(Player player, int weaponId) {
		...
		case LUNAR:
			resetAutoCastInterface(player);
			player.send(new SendMessage("You can't autocast on this spellbook."));
			break;
		case MODERN:
