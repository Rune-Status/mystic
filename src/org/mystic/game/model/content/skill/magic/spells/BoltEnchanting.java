	...
	public static void handle(Player player, int itemId) {
		...
		if (player.getSkill().getLevels()[Skills.MAGIC] < data.getLevel()) {
			player.send(new SendMessage(
					"You need a Magic level of " + data.getLevel() + " to enchant " + boltName + " bolts."));
			return;
		}
		if (!player.getInventory().hasItemAmount(data.getBolt(), 10)) {
			player.send(new SendMessage("You need atleast 10 " + boltName + " to enchant."));
			return;
		}
		if (!player.getInventory().hasAllItems(data.getRunes())) {
			player.send(new SendMessage("You do not have the correct runes needed to do this."));
			player.send(new SendRemoveInterfaces());
			return;
		}
		...
		player.send(
				new SendMessage("You have enchanted 10 " + boltName + (boltName.endsWith("bolts") ? "." : " bolts.")));
		String color = "@red@";
		...
		player.send(new SendRemoveInterfaces());
		player.getSkill().addExperience(Skills.MAGIC, 10 * GameConstants.EXPERIENCE_RATE);

	}

	private static int[] levels = { 4, 7, 14, 24, 27, 29, 49, 57, 68, 87 };

	private static int[][] runes = { { 564, 1 }, { 556, 2 }, { 564, 1 }, { 555, 1 }, { 558, 1 }, { 564, 1 }, { 557, 2 },
			{ 564, 1 }, { 555, 2 }, { 564, 1 }, { 556, 3 }, { 561, 1 }, { 564, 1 }, { 554, 2 }, { 564, 1 }, { 554, 5 },
			{ 565, 1 }, { 564, 1 }, { 557, 10 }, { 563, 2 }, { 564, 1 }, { 557, 15 }, { 566, 1 }, { 564, 1 },
			{ 554, 20 }, { 560, 1 } };

	private static Item[] item = { new Item(9236), new Item(9240), new Item(9237), new Item(9238), new Item(9241),
			new Item(9239), new Item(9242), new Item(9243), new Item(9244), new Item(9245) };
