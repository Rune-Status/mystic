...
public enum FinishedPotionData {
	...
	PRAYER_POTION(new Item(139), new Item(99), new Item(231), 38, 88),
	...
	public static Optional<FinishedPotionData> get(Item item, Item item2) {
		return POTION_VALUES.stream().filter(Objects::nonNull)
				.filter(potions -> potions.getUnfinishedPotion().getId() == item.getId())
				.filter(potions -> potions.getIngredient().getId() == item2.getId()).findFirst();
	}
