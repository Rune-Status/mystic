package org.mystic.game.model.content.skill.fletching;
...
import org.mystic.game.model.content.skill.firemaking.Log;
import org.mystic.game.model.content.skill.fletching.fletchable.Fletchable;
import org.mystic.game.model.content.skill.fletching.fletchable.FletchableItem;
import org.mystic.game.model.content.sound.SoundPlayer;
import org.mystic.game.model.entity.Animation;
import org.mystic.game.model.entity.item.Item;
import org.mystic.game.model.entity.player.Player;
import org.mystic.game.model.networking.outgoing.SendChatBoxInterface;
...
public enum Fletching {
	...
	public boolean itemOnItem(Player player, Item use, Item with) {
		...
		if (!player.getInventory().playerHasItem(new Item(fletchable.getUse().getId()))) {
			player.send(new SendMessage(
					"You need a " + fletchable.getUse().getDefinition().getName().toLowerCase() + " to do that."));
			return false;
		}
		if (!player.getInventory().playerHasItem(new Item(fletchable.getWith().getId()))) {
			player.send(new SendMessage(
					"You need a " + fletchable.getWith().getDefinition().getName().toLowerCase() + " to do that."));
			return false;
		}
	...
	public boolean fletch(Player player, int index, int amount) {
		...
		final Fletchable fletchable = (Fletchable) player.getAttributes().get(FLETCHABLE_KEY);
		if (fletchable == null) {
			return false;
		}
	...
	public boolean start(Player player, Fletchable fletchable, int index, int amount) {
		if (Objects.isNull(fletchable)) {
			return false;
		}
		player.getAttributes().remove(FLETCHABLE_KEY);
		final FletchableItem item = fletchable.getFletchableItems()[index];
		player.send(new SendRemoveInterfaces());
		if (player.getLevels()[Skills.FLETCHING] < item.getLevel()) {
			DialogueManager.sendStatement(player, "You need a Fletching level of " + item.getLevel() + " to do that.");
			return true;
		}
		if (!(player.getInventory().hasAllItems(fletchable.getIngediants()))) {
			...
			if (fletchable.getUse().getAmount() == 1) {
				firstAmount = Misc.getAOrAn(fletchable.getUse().getDefinition().getName());
			} else {
				firstAmount = String.valueOf(fletchable.getUse().getAmount());
			}
			final String secondAmount;
			if (fletchable.getWith().getAmount() == 1) {
				secondAmount = Misc.getAOrAn(fletchable.getWith().getDefinition().getName());
			} else {
		...
		TaskQueue.queue(
				new Task(player, 3, true, StackType.NEVER_STACK, BreakType.ON_MOVE, TaskIdentifier.CURRENT_ACTION) {
					...
					public void execute() {
						...
						SoundPlayer.play(player, 375, 0, 0);
						if (item.getProduct().getDefinition().getName().contains("bow")
								|| item.getProduct().getDefinition().getName().contains("shaft")
								|| item.getProduct().getDefinition().getName().contains("stock")) {
						...
						if (!(player.getInventory().hasAllItems(fletchable.getIngediants()))) {
							stop();
							player.send(new SendMessage("You have ran out of materials."));
							return;
						}
					}
