...
import org.mystic.game.model.content.skill.crafting.craftable.Craftable;
import org.mystic.game.model.content.skill.crafting.craftable.CraftableItem;
...
public enum HideCrafting {
	...
	public boolean itemOnItem(Player player, Item use, Item with) {
		...
		if (!player.getInventory().playerHasItem(new Item(1733))
				&& !player.getInventory().playerHasItem(new Item(1734))) {
			return false;
		}
		...
		case 2:
			...
			player.send(new SendString("\\n \\n \\n \\n \\n ".concat(
					craftable.getCraftableItems()[0].getProduct().getDefinition().getName().replace("d'hide ", "")),
					8874));
			player.send(new SendString("\\n \\n \\n \\n \\n ".concat(
					craftable.getCraftableItems()[1].getProduct().getDefinition().getName().replace("d'hide ", "")),
					8878));
		...
		case 3:
			...
			player.send(new SendString("\\n \\n \\n \\n \\n".concat(
					...
					8893));
			player.send(new SendString("\\n \\n \\n \\n \\n".concat(
					craftable.getCraftableItems()[2].getProduct().getDefinition().getName().replace("d'hide ", "")),
					8897));
		...
		case 4:
			...
			player.send(new SendString("\\n \\n \\n \\n \\n".concat(
					...
					8917));
			player.send(new SendString("\\n \\n \\n \\n \\n".concat(
					craftable.getCraftableItems()[3].getProduct().getDefinition().getName().replace("d'hide ", "")),
					8921));
	...
	public boolean craft(Player player, int index, int amount) {
		...
		final Craftable craftable = (Craftable) player.getAttributes().get(CRAFTABLE_KEY);
		if (craftable == null) {
			return false;
		}
	...
	public boolean start(Player player, Craftable craftable, int index, int amount) {
		...
		final CraftableItem item = craftable.getCraftableItems()[index];
		player.send(new SendRemoveInterfaces());
		if (player.getLevels()[Skills.CRAFTING] < item.getLevel()) {
			DialogueManager.sendStatement(player, "You need a crafting level of " + item.getLevel() + " to make that.");
			return true;
		}
		if (!player.getInventory().hasAllItems(craftable.getIngediants(index))) {
			player.send(new SendMessage("You don't have the required items needed to make this."));
			return true;
		}
		TaskQueue.queue(
				new Task(player, 2, false, StackType.NEVER_STACK, BreakType.ON_MOVE, TaskIdentifier.SKILL_CREATING) {
					...
					public void execute() {
						...
						if (!player.getInventory().hasAllItems(craftable.getIngediants(index))) {
							stop();
							player.send(new SendMessage("You have run out of hide."));
							return;
						}
					}
