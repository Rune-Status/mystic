		...
		for (FarmingPatches patch : FarmingPatches.values()) {
			if ((x >= patch.x) && (y >= patch.y) && (x <= patch.x2) && (y <= patch.y2)) {
				if (!patches[patch.ordinal()].isRaked()) {
					player.getClient()
							.queueOutgoingPacket(new SendMessage("You must remove the weeds before planting."));
					return true;
				}
				for (Plants plant : Plants.values()) {
					if (plant.seed == seed) {
						if (player.getMaxLevels()[Skills.FARMING] >= plant.level) {
							if (inhabited(x, y)) {
								player.getClient().queueOutgoingPacket(
