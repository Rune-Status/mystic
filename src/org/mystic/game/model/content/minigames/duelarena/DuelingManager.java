...
public class DuelingManager {
	...
	public static Location getCoordinates(int arenaId, boolean obstacles, boolean secondPlayer, boolean noMovement) {
		if (noMovement) {
			...
			return new Location(p.getX() + (5 + Misc.randomNumber(5)) * (Misc.randomNumber(1) == 0 ? 1 : -1),
					p.getY() + Misc.randomNumber(6) * (Misc.randomNumber(1) == 0 ? 1 : -1));
		}
		if (obstacles) {
		...
		Location p = NON_OBSTACLE_ARENA_LOCATIONS[arenaId];
		return new Location(p.getX() + (5 + Misc.randomNumber(5)) * (secondPlayer ? 1 : -1),
				p.getY() + Misc.randomNumber(5) * (Misc.randomNumber(1) == 0 ? 1 : -1));
	}
