...
public class Trade {
	...
	public String getTotalAmount(int amount) {
	...
	public int getTradedWealth(Player Player) {
		int value = 0;
	...
	public void sendItemText(Player player) {
		final Item[] traded = player.getTrade().getTradedItems();
		final Item[] recieving = player.getTrade().getTradingWith().getTradedItems();
	...
	private TradeStages stage = TradeStages.NONE;

	private Trade tradingWith = null;

	private TradeContainer container = new TradeContainer(this);
	...
	public void begin(final Trade tradingWith) {

		player.getClient().queueOutgoingPacket(new SendString(
				getStatus() + " with: " + NameUtil.uppercaseFirstLetter(tradingWith.getPlayer().getUsername()), 3417));
		tradingWith.getPlayer().getClient().queueOutgoingPacket(
				new SendString(getStatus() + " with: " + NameUtil.uppercaseFirstLetter(player.getUsername()), 3417));
	...
	public void end(boolean success) {
		final Item[] traded = getTradedItems();
		final Item[] recieving = tradingWith.getTradedItems();
		final HashMap<Integer, Integer> trade = new HashMap<>();
		final HashMap<Integer, Integer> recieved = new HashMap<>();
		for (int i = 0; i < 28; i++) {
			if (success) {
				if (traded[i] != null) {
				...
				if (recieving[i] != null) {
					...
					if (recieved.get(recieving[i].getId()) != null) {
						recieved.put(recieving[i].getId(),
		...
		for (int item : trade.keySet()) {
			...
			strings[0][index] = new String[] { Misc.formatPlayerName(player.getUsername()), "" + tradedItem.getAmount(),
					Misc.formatPlayerName(tradedItem.getDefinition().getName()),
					Misc.formatPlayerName(tradingWith.player.getUsername()) };
			strings[1][index] = new String[] { Misc.formatPlayerName(tradingWith.player.getUsername()),
					"" + tradedItem.getAmount(), Misc.formatPlayerName(tradedItem.getDefinition().getName()),
					Misc.formatPlayerName(player.getUsername()) };
			index++;
		}
		PlayerLogs.multiLog(player.getUsername(), "%s has given %s %s to %s", strings[0]);
		PlayerLogs.multiLog(tradingWith.player.getUsername(), "%s has received %s %s from %s", strings[1]);
		strings = new String[2][recieved.size()][4];
		index = 0;
		for (int item : recieved.keySet()) {
			Item tradedItem = new Item(item, recieved.get(item));
			strings[0][index] = new String[] { Misc.formatPlayerName(tradingWith.player.getUsername()),
					"" + tradedItem.getAmount(), Misc.formatPlayerName(tradedItem.getDefinition().getName()),
					Misc.formatPlayerName(player.getUsername()) };
			strings[1][index] = new String[] { Misc.formatPlayerName(player.getUsername()), "" + tradedItem.getAmount(),
					Misc.formatPlayerName(tradedItem.getDefinition().getName()),
					Misc.formatPlayerName(tradingWith.player.getUsername()) };
			index++;
		}
		PlayerLogs.multiLog(tradingWith.player.getUsername(), "%s has given %s %s to %s", strings[0]);
		PlayerLogs.multiLog(player.getUsername(), "%s has received %s %s from %s", strings[1]);
		if (success) {
	...
	public void request(Player requested) {
		...
		if (IronMan.isIronMan(requested)) {
			player.send(new SendMessage("This player is an iron man and can not trade."));
			return;
		}
		...
		if (requested.getTrade().requested(player)) {
			...
			requested.getClient().queueOutgoingPacket(new SendMessage(
					NameUtil.uppercaseFirstLetter(player.getUsername()) + ":" + getRequestString() + "req:"));
		}
	}
