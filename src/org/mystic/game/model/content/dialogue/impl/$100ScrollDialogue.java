...
public class $100ScrollDialogue extends Dialogue {

	public $100ScrollDialogue(Player player) {
		this.player = player;
	}
	...
	public void execute() {
		...
		case 0:
			DialogueManager.sendNpcChat(player, 590, Emotion.HAPPY_TALK,
					"Activating this scroll will grant you 1,000 Member Points.", "Are you sure you want to do this?");
			next++;
			break;
		case 1:
		...
		case 2:
			...
			break;
		case 3:
			if (player.getInventory().playerHasItem(new Item(608, 1))) {
				player.getInventory().remove(608, 1);
				player.addDonatorPoints(1000);
				QuestTab.update(player);
				player.send(new SendMessage("Your account has been credited 1,000 Member Points."));
				player.getDialogue().end();
				player.getClient().queueOutgoingPacket(new SendRemoveInterfaces());
			} else {
				player.getDialogue().end();
