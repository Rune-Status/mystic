...
public class ItemDegrading {
	...
	public class DegradedItem {
		...
		public DegradedItem(final int id, final int hits, final int maxHits) {
			this.id = id;
			this.hits = hits;
	...
	public void degradeEquipment(Player player) {
		for (int i = 0; i < 14; i++)
			if (i != 3) {
				...
				if (item != null) {
					...
					if (ref == null) {
						...
						if (index != -1) {
							...
							player.getEquipment().getItems()[i].setId(EQUIPMENT_DEGRADING[index][1]);
			...
			if (ref != null) {
				...
				if (isFullDegraded(ref)) {
					...
					if (lastStage == -1) {
						...
						p.getClient().queueOutgoingPacket(new SendMessage("Your " + weapon.getDefinition().getName()
