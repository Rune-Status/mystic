package org.mystic.game.model.entity.following;
...
public abstract class Following {
		...
		for (int k = 0; k < 4; k++) {
			for (int i = 0; i < loop - (k == 0 ? 1 : 0); i++) {
				...
				if ((d = Misc.getManhattanDistance(x3, y3, x, y)) < lowDist) {
					if ((RS317PathFinder.accessable(x2, y2, z, x3, y3)) && (((!projectile)
							&& (StraightPathFinder.isInteractionPathClear(x3, y3, z, x2, y2)))
							|| ((projectile) && (StraightPathFinder.isProjectilePathClear(x3, y3, z, x2, y2))))) {
						lowDist = d;
						lowX = x3;
						lowY = y3;
	...
	public Location getFollowToLocation() {
		...
		int[][] nodes = {
				{ x + org.mystic.game.GameConstants.DIR[NON_DIAGONAL_DIRECTIONS[0]][0],
						y + org.mystic.game.GameConstants.DIR[NON_DIAGONAL_DIRECTIONS[0]][1] },
				{ x + org.mystic.game.GameConstants.DIR[NON_DIAGONAL_DIRECTIONS[1]][0],
						y + org.mystic.game.GameConstants.DIR[NON_DIAGONAL_DIRECTIONS[1]][1] },
				{ x + org.mystic.game.GameConstants.DIR[NON_DIAGONAL_DIRECTIONS[2]][0],
						y + org.mystic.game.GameConstants.DIR[NON_DIAGONAL_DIRECTIONS[2]][1] },
				{ x + org.mystic.game.GameConstants.DIR[NON_DIAGONAL_DIRECTIONS[3]][0],
						y + org.mystic.game.GameConstants.DIR[NON_DIAGONAL_DIRECTIONS[3]][1] } };
