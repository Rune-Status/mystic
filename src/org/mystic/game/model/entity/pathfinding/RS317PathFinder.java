...
public class RS317PathFinder {
	...
	public static void findRoute(Player player, int destX, int destY, boolean moveNear, int xLength, int yLength) {
		if (destX == player.getLocation().getLocalX() && destY == player.getLocation().getLocalY() && !moveNear) {
			return;
		}
		destX = destX - (player.getLocation().getRegionX() << 3);
		destY = destY - (player.getLocation().getRegionY() << 3);
		int[][] via = new int[104][104];
		int[][] cost = new int[104][104];
		...
		tileQueueX.add(curX);
		tileQueueY.add(curY);
		final int regionX = player.getLocation().getRegionX() << 3;
		final int regionY = player.getLocation().getRegionY() << 3;
		...
		while (tail != tileQueueX.size() && tileQueueX.size() < pathLength) {
			...
			if (curY > 0 && via[curX][curY - 1] == 0
					&& (getClip(curAbsX, curAbsY - 1, player.getLocation().getZ()) & 0x1280102) == 0) {
				tileQueueX.add(curX);
				tileQueueY.add(curY - 1);
				via[curX][curY - 1] = 1;
				cost[curX][curY - 1] = thisCost;
			...
			if (curX > 0 && via[curX - 1][curY] == 0
					&& (getClip(curAbsX - 1, curAbsY, player.getLocation().getZ()) & 0x1280108) == 0) {
				tileQueueX.add(curX - 1);
				tileQueueY.add(curY);
				via[curX - 1][curY] = 2;
				cost[curX - 1][curY] = thisCost;
			...
			if (curY < 104 - 1 && via[curX][curY + 1] == 0
					&& (getClip(curAbsX, curAbsY + 1, player.getLocation().getZ()) & 0x1280120) == 0) {
				tileQueueX.add(curX);
				tileQueueY.add(curY + 1);
				via[curX][curY + 1] = 4;
				cost[curX][curY + 1] = thisCost;
			...
			if (curX < 104 - 1 && via[curX + 1][curY] == 0
					&& (getClip(curAbsX + 1, curAbsY, player.getLocation().getZ()) & 0x1280180) == 0) {
				tileQueueX.add(curX + 1);
				tileQueueY.add(curY);
				via[curX + 1][curY] = 8;
				cost[curX + 1][curY] = thisCost;
			...
			if (curX > 0 && curY > 0 && via[curX - 1][curY - 1] == 0
					&& (getClip(curAbsX - 1, curAbsY - 1, player.getLocation().getZ()) & 0x128010e) == 0
					&& (getClip(curAbsX - 1, curAbsY, player.getLocation().getZ()) & 0x1280108) == 0
					&& (getClip(curAbsX, curAbsY - 1, player.getLocation().getZ()) & 0x1280102) == 0) {
				tileQueueX.add(curX - 1);
				tileQueueY.add(curY - 1);
			...
			if (curX > 0 && curY < 104 - 1 && via[curX - 1][curY + 1] == 0
					&& (getClip(curAbsX - 1, curAbsY + 1, player.getLocation().getZ()) & 0x1280138) == 0
					&& (getClip(curAbsX - 1, curAbsY, player.getLocation().getZ()) & 0x1280108) == 0
					&& (getClip(curAbsX, curAbsY + 1, player.getLocation().getZ()) & 0x1280120) == 0) {
				tileQueueX.add(curX - 1);
				tileQueueY.add(curY + 1);
			...
			if (curX < 104 - 1 && curY > 0 && via[curX + 1][curY - 1] == 0
					&& (getClip(curAbsX + 1, curAbsY - 1, player.getLocation().getZ()) & 0x1280183) == 0
					&& (getClip(curAbsX + 1, curAbsY, player.getLocation().getZ()) & 0x1280180) == 0
					&& (getClip(curAbsX, curAbsY - 1, player.getLocation().getZ()) & 0x1280102) == 0) {
				tileQueueX.add(curX + 1);
				tileQueueY.add(curY - 1);
			...
			if (curX < 104 - 1 && curY < 104 - 1 && via[curX + 1][curY + 1] == 0
					&& (getClip(curAbsX + 1, curAbsY + 1, player.getLocation().getZ()) & 0x12801e0) == 0
					&& (getClip(curAbsX + 1, curAbsY, player.getLocation().getZ()) & 0x1280180) == 0
					&& (getClip(curAbsX, curAbsY + 1, player.getLocation().getZ()) & 0x1280120) == 0) {
				tileQueueX.add(curX + 1);
				tileQueueY.add(curY + 1);
		...
		player.getMovementHandler().addToPath(new Location(pathX, pathY));
