...
/**
 * Holds all the calculation for Player vs Mob combat
 * 
 * @author Vali - http://www.rune-server.org/members/Valiant
 *
 */
public class NpcCombatFormulas {
	...
	public static int getAccuracy(Entity attacking, Entity defending, CombatTypes combatType) {
		...
		if (!defending.isNpc()) {
			pDefender = org.mystic.game.World.getPlayers()[defending.getIndex()];
		}
		if (!attacking.isNpc()) {
			pAttacker = org.mystic.game.World.getPlayers()[attacking.getIndex()];
				...
				for (int i = 0; i < DEFENCE_CURSE_PRAYER_MODIFIERS.length; i++) {
					if (pDefender.getPrayer().active((int) DEFENCE_CURSE_PRAYER_MODIFIERS[i][0])) {
						...
						if (defending.getMaxLevels()[1] > 99) {
							dDefensive = (int) (dDefensive + 18.85D);
							break;
						}
		...
		if ((!defending.isNpc()) && (!attacking.isNpc())) {
			dDefensive = (int) (dDefensive * 0.95D);
		}
		switch (combatType) {
		case MAGIC:
			...
			if (pAttacker != null) {
				...
				if ((helm != null) && (helm.getId() == 15492) && (defending.isNpc())
					...
					if ((m != null) && pAttacker.getSlayer().getTask() == m.getId()) {
		...
		case RANGED:
			...
			if (pAttacker != null) {
				...
				if (isDiamondBoltSpec(pAttacker)) {
					dBonus = (int) (defending.getBonuses()[9] * 0.75D);
				}
				if (pAttacker.getPrayer().getPrayerBookType() == PrayerBook.PrayerBookType.DEFAULT) {
		...
		if (combatType == CombatTypes.MAGIC) {
			...
			eD = (int) (m * 0.3D + dDefensive * dPBonus * 0.8D);
		} else {
		...
		if ((pAttacker != null) && (combatType != CombatTypes.MAGIC)
				&& (pAttacker.getSpecialAttack().isInitialized())) {
	...
	public static double getDefenderPrayerModifier(Entity e1, Entity e2, CombatTypes type) {
		if ((e2 != null) && (!e2.isNpc())) {
			Player pDefender = org.mystic.game.World.getPlayers()[e2.getIndex()];
			if ((pDefender != null) && (pDefender.getPrayer().getPrayerBookType() == PrayerBook.PrayerBookType.CURSES)
					&& (((pDefender.getPrayer().active(1)) && ((type == CombatTypes.MELEE) || (type == null)))
							|| ((pDefender.getPrayer().active(2)) && ((type == CombatTypes.RANGED) || (type == null)))
							|| ((pDefender.getPrayer().active(3))
									&& ((type == CombatTypes.MAGIC) || (type == null))))) {
