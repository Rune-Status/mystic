...
public class Walking {
	...
	public static boolean canMoveTo(Npc mob, int direction) {
		...
		VirtualNpcRegion region = mob.getVirtualRegion();
		...
		for (int i = 1; i < size + 1; i++) {
			for (int k = 0; k < GameConstants.SIZES[i].length; k++) {
				...
				for (int j = 0; j < 8; j++) {
					if ((GameConstants.withinBlock(x5, y5, size, x2 + GameConstants.DIR[j][0],
							x2 + GameConstants.DIR[j][1])) && (!Region.getRegion(x2, x2).canMove(x2, x2, z, j))) {
						return false;
					}
				}
			}
	...
	public static boolean canMoveTo(Npc mob, int x, int y, int direction, int size) {
		if (direction == -1) {
			return false;
		}
	...
	public static int getRandomDirection(Npc mob) {
		int count = 0;
	...
	public static void randomWalk(Npc mob) {
		if (mob.getId() == 6064 || mob.getId() == 6063 || mob.getId() == 1035 || mob.getId() == 1034
				|| mob.getId() == 1033 || mob.getId() == 1032 || mob.getId() == 1031 || mob.getId() == 1030
				|| mob.getId() == 1029 || mob.getId() == 1028) {
		...
		} else if (Misc.randomNumber(15) == 0) {
			int dir = getRandomDirection(mob);
			if (dir != -1)
				SimplePathWalker.walkToNextTile(mob,
						new Location(mob.getX() + GameConstants.DIR[dir][0], mob.getY() + GameConstants.DIR[dir][1]));
		}
	}

	public static void setNpcOnTile(Npc mob, boolean set) {
		int x = mob.getLocation().getX();
		int y = mob.getLocation().getY();
		int z = mob.getLocation().getZ() > 3 ? mob.getLocation().getZ() % 4 : mob.getLocation().getZ();
	...
	public static void walk(Npc mob, int dir) {
	...
	public static boolean withinMaxDistance(Npc mob, int direction) {
		int yMax;
		int xMax;
		int yMin;
		int xMin;
