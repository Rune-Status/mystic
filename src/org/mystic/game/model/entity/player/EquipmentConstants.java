...
import org.mystic.game.model.entity.player.Equipment.AttackStyles;
import org.mystic.utility.GameDefinitionLoader;

public class EquipmentConstants {

	public enum WeaponAttackStyles {
	...
	private static final int[] FULL_BODY_IDS = { 7390, 7392, 14936, 10460, 10458, 10462, 2916, 20096, 10370, 10386,
			10378, 6129, 7399, 10564, 11720, 10386, 1035, 13624, 13614, 577, 9674, 6107, 1115, 1117, 1119, 1121, 1123,
			1125, 1127, 2583, 2584, 2591, 2599, 2607, 2615, 2623, 2653, 2661, 2669, 3481, 4720, 4728, 4728, 4749, 4892,
			4894, 4895, 4916, 4917, 4918, 4919, 4964, 4965, 4966, 4967, 6617, 10348, 14479, 10551, 4736, 4940, 4941,
			4942, 4943, 11724, 544, 4091, 4101, 4111, 4991, 4990, 4989, 4988, 4758, 4712, 4868, 4869, 4870, 4871, 4757,
			8839, 3058, 6654, 6180, 6184, 17259, 5575, 3140, 10748, 14595, 11020, 10338, 10400, 10404, 10408, 10412,
			10416, 10420, 13619, 6916, 13887, 13884, 13858, 13870, 13889, 13886, 13860, 13872, 14497, 14601, 15600,
			15606, 15612, 15618, 17193, 9944, 14114, 12873, 12894, 12971 };

	private static final int[] FULL_HELM_IDS = { 20751, 20096, 20098, 20100, 20102, 20104, 20094, 10342, 20100, 10589,
			4724, 1153, 1155, 1157, 1159, 1161, 1163, 1165, 2587, 2595, 2605, 2613, 2619, 2627, 2657, 2665, 2673, 3486,
			6623, 10350, 11335, 13896, 13898, 4724, 4904, 4905, 4906, 4907, 4716, 4880, 4881, 4882, 4883, 4708, 4856,
			4857, 4858, 4859, 4745, 4952, 4953, 4954, 4955, 4732, 4928, 4929, 4930, 4931, 7534, 11718, 6109, 3748, 3749,
			3751, 3753, 3755, 11663, 11664, 11665, 4753, 4976, 4977, 4978, 4979, 4709, 4856, 4857, 4858, 4859, 10828,
			3057, 7594, 6188, 1149, 10548, 10547, 16711, 5574, 4502, 11021, 10334, 10392, 10398, 13876, 15492, 10589,
			1053, 1055, 1057, 17061, 13263, 14116, 1167, 12673, 12675, 12677, 12679, 12681 };

	private static final int[] FULL_MASK_IDS = { 8464, 8466, 8468, 8470, 8472, 8474, 8476, 8478, 8480, 8482, 8484, 8486,
			8488, 8490, 8492, 4745, 4724, 1153, 1155, 1157, 1159, 1161, 1163, 1165, 2587, 2595, 2605, 2613, 2619, 2627,
			2657, 2665, 2673, 3486, 6623, 10350, 11335, 4979, 4978, 4977, 4976, 4753, 3057, 7594, 6188, 5574, 15492,
			1053, 1055, 1057, 13263, };
	...
	public static void clickAttackTypeButtons(Player player, int buttonId) {
		switch (buttonId) {
		case 1080:
		case 1079:
		case 1078:
	...
	public static boolean clickAttackStyleButtons(Player player, int buttonId) {
		clickAttackTypeButtons(player, buttonId);
		switch (buttonId) {
		// aggressive
		case 6236:
		case 9125:
	...
	static {
		...
		for (int i = 0; i < 20145; i++) {
			...
			if ((def1 != null) && (def2 != null)) {
				...
				if ((def2.getName() != null) && (def2.getName().contains("hood")) && (!def2.getName().contains("Robin"))
