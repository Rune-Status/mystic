...
public final class PlayerUpdating {
	...
	public static void appendChat(PlayerUpdateFlags flags, StreamBuffer.OutBuffer out) {
		out.writeShort(((flags.getChatColor() & 0xFF) << 8) + (flags.getChatEffects() & 0xFF),
	...
	public static void appendAppearance(PlayerUpdateFlags flags, StreamBuffer.OutBuffer out) {
		...
		if (flags.getNpcAppearanceId() == -1) {
			if (e[EquipmentConstants.HELM_SLOT] != 0)
				block.writeShort(512 + e[EquipmentConstants.HELM_SLOT]);
			else {
				block.writeByte(0);
			}
			if (e[EquipmentConstants.CAPE_SLOT] != 0)
				block.writeShort(512 + e[EquipmentConstants.CAPE_SLOT]);
			else {
				block.writeByte(0);
			}
			if (e[EquipmentConstants.NECKLACE_SLOT] != 0)
				block.writeShort(512 + e[EquipmentConstants.NECKLACE_SLOT]);
			else {
				block.writeByte(0);
			}
			if (e[EquipmentConstants.WEAPON_SLOT] != 0)
				block.writeShort(512 + e[EquipmentConstants.WEAPON_SLOT]);
			else {
				block.writeByte(0);
			}
			if (e[EquipmentConstants.TORSO_SLOT] != 0)
				block.writeShort(512 + e[EquipmentConstants.TORSO_SLOT]);
			else {
				block.writeShort(256 + flags.getAppearance()[1]);
			}
			if (e[EquipmentConstants.SHIELD_SLOT] != 0)
				block.writeShort(512 + e[EquipmentConstants.SHIELD_SLOT]);
			else {
				block.writeByte(0);
			}
			if ((e[EquipmentConstants.TORSO_SLOT] != 0)
					&& (EquipmentConstants.isFullBody(e[EquipmentConstants.TORSO_SLOT])))
				block.writeByte(0);
			else {
				block.writeShort(256 + flags.getAppearance()[2]);
			}
			if (e[EquipmentConstants.LEGS_SLOT] != 0)
				block.writeShort(512 + e[EquipmentConstants.LEGS_SLOT]);
			else {
				block.writeShort(256 + flags.getAppearance()[4]);
			}
			if (((e[EquipmentConstants.HELM_SLOT] != 0)
					&& (EquipmentConstants.isFullHelm(e[EquipmentConstants.HELM_SLOT])))
					|| e[EquipmentConstants.HELM_SLOT] != 0
							&& (EquipmentConstants.isFullMask(e[EquipmentConstants.HELM_SLOT]))) {
				block.writeByte(0);
			} else if ((e[EquipmentConstants.HELM_SLOT] != 0)
					&& (EquipmentConstants.isForceNewHair(e[EquipmentConstants.HELM_SLOT])))
				block.writeShort(259);
			else {
				block.writeShort(256 + flags.getAppearance()[0]);
			}
			if (e[EquipmentConstants.GLOVES_SLOT] != 0)
				block.writeShort(512 + e[EquipmentConstants.GLOVES_SLOT]);
			else {
				block.writeShort(256 + flags.getAppearance()[3]);
			}
			if (e[EquipmentConstants.BOOTS_SLOT] != 0)
				block.writeShort(512 + e[EquipmentConstants.BOOTS_SLOT]);
			else {
				block.writeShort(256 + flags.getAppearance()[5]);
			}
			if (flags.getGender() == 1) {
				block.writeByte(0);
			} else if ((e[0] != 0) && (EquipmentConstants.isFullMask(e[EquipmentConstants.HELM_SLOT])))
				block.writeByte(0);
			else
		...
		block.writeString(flags.getTitle());
	...
	public static void addPlayer(StreamBuffer.OutBuffer out, PlayerUpdateFlags local, PlayerUpdateFlags other,
			int index) {
