...
public abstract class StreamBuffer {
	...
	/**
	 * An enum whose values represent the current type of access to a StreamBuffer.
	 * BYTE_ACCESS is for reading/writing bytes, BIT_ACCESS is for reading/writing
	 * bits.
	 * 
	 * @author blakeman8192
	 */
	...
	/**
	 * An enum whose values represent the possible order in which bytes are written
	 * in a multiple-byte value. Also known as "endianness".
	 * 
	 * @author blakeman8192
	 */
	...
	public static final class InBuffer extends StreamBuffer {
		...
		/**
		 * Reads the amuont of bytes into the array, starting at the current location.
		 * 
		 * @param amount
		 *            the amount to read
		 ...
		 * location + amount and reading in reverse until the current location.
		 * 
		 * @param amount
		 *            the amount of bytes
		 * @param type
	...
	public static final class OutBuffer extends StreamBuffer {
		...
		/**
		 * Finishes a variable packet header by writing the actual packet length at the
		 * length byte's location. Call this when the construction of the actual
		...
		/**
		 * Finishes a variable packet header by writing the actual packet length at the
		 * length short's location. Call this when the construction of the variable
		 * length packet is complete.
		 */
		 ...
		 * not modify the argued buffer, and please do not flip() the buffer before
		 * hand.
		 * 
		 * @param from
		 */
		 ...
		 * corresponding "finishVariablePacketHeader" must be called to finish the
		 * packet.
		 * 
		 * @param cipher
		...
		/**
		 * Writes a packet header for a variable length packet, where the length is
		 * written as a short instead of a byte. Note that the corresponding
	 ...
	 * Type A is to add 128 to the value, type C is to invert the value, and type S
	 * is to subtract the value from 128. Of course, STANDARD is just the normal
	 * data value.
	 * 
	 * @author blakeman8192
	 */
