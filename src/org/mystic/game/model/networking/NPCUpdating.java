...
public class NPCUpdating {
	...
	public static void addNewNpc(StreamBuffer.OutBuffer out, Player player, Npc mob) {
		...
		out.writeBits(1, 0);
		out.writeBits(18, mob.getId());
		out.writeBit(true);
	}

	public static void appendState(StreamBuffer.OutBuffer block, NpcUpdateFlags npc, int faceDir) {
		int mask = 0x0;
		...
		if (npc.isHitUpdate()) {
			...
			block.writeByte(100);
			block.writeByte(npc.getHitUpdateCombatType());
		}
		if (npc.isGraphicsUpdateRequired()) {
			block.writeShort(npc.getGraphicId());
			block.writeInt(npc.getGraphicDelay() | npc.getGraphicHeight() << 16);
		}
		if (npc.isEntityFaceUpdate()) {
		...
		if (npc.isFaceToDirection() || faceDir != -1) {
			...
			if (npc.isFaceToDirection()) {
				...
				pos = new Location(npc.getLocation().getX() + GameConstants.DIR[faceDir][0],
	...
	public static void update(Client client, PlayerUpdateFlags pFlags, NpcUpdateFlags[] nFlags) {
		...
		for (Iterator<Npc> i = client.getNpcs().iterator(); i.hasNext();) {
			Npc mob = i.next();
			NpcUpdateFlags flags = nFlags[mob.getIndex()];

			if (mob.getLocation().isViewableFrom(pFlags.getLocation()) && mob.isVisible() && flags != null
					&& !flags.isPlacement()) {
		...
		for (int i = 0; i < World.getNpcs().length; i++) {
			...
			Npc mob = World.getNpcs()[i];
			NpcUpdateFlags flags = nFlags[i];
	...
	public static void updateNpcMovement(StreamBuffer.OutBuffer out, NpcUpdateFlags npc) {
