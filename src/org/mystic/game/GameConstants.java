...
public class GameConstants {
	...
	/**
	 * Experience rates for combat
	 */
	public static final int EXPERIENCE_RATE = 1200;

	/**
	 * Experience rates for skilling
	 */
	public static final int SKILL_RATE = 50;
	 ...
	 */
	public static final int[] BLOCKED_DOORS = { 26428, 26426, 26427, 26425, 26502, 26503, 26504, 26505, 24306, 24309,
			26760, 2104, 2102, 2100, 26461, 4799, 7129 };
	...
	public static Location[] getBorder(int x, int y, int xsize, int ysize) {
		...
		border[0] = new Location(x, y);
		for (int i = 0; i < 4; i++) {
			for (int k = 0; k < (i < 3 ? (i == 0 || i == 2 ? xsize : ysize) - 1
					: (i == 0 || i == 2 ? xsize : ysize) - 2); k++) {
				if (i == 0)
					x++;
				else if (i == 1)
					y++;
	...
	public static Location getClearAdjacentLocation(Location l, int size, VirtualNpcRegion virtual) {
		int x = l.getX();
		int y = l.getY();
		int z = l.getZ();
		int lowDist = 99999;
	...
	public static boolean withinBlock(int blockX, int blockY, int blockSize, int x, int y, int checkSize) {
		for (int i = 1; i < checkSize + 1; i++) {
			for (int k = 0; k < SIZES[i].length; k++) {
				...
				if ((x2 - blockX < blockSize) && (x2 - blockX > -1) && (y2 - blockY < blockSize)
						&& (y2 - blockY > -1)) {
					return true;
				}
			}
